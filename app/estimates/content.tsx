...
function Estimates() {
    ...
    const { data } = useSWR(`${process.env.NEXT_PUBLIC_URL}/api/estimates/table?page=${page}`, fetcher);

    const debouncedSearch = React.useRef(
        debounce(async (e: React.ChangeEvent<HTMLInputElement>) => {
            try {
                const res = await fetch(`${process.env.NEXT_PUBLIC_URL}/api/estimates/search`, {
                    method: "POST",
                    headers: {
    ...
    if (!data)
        return (
            <Appshell>
                <Head>
                    <title>Devis | Orderise</title>
                </Head>
                <AppTableHead
                    title="Devis"
                    currentPage="Liste"
                    btnLabel="Nouveau devis"
                    btnLink="/estimates/create"
                    count={0}
                />
    ...
    return (
        <Appshell>
            ...
            <DataTable columns={columns} data={searchResult ?? data.estimates} />
            <div className="flex items-center justify-end space-x-2 py-4">
