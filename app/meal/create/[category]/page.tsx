...
function CreateMealPage({ params: { category } }: { params: { category: string } }) {
    ...
    const [meal, setMeal] = React.useState({
        ...
        mealCode: "",
        mealPrice: 0,
        mealCategory: `meal-${category === "oysters" ? "huitres" : category}`,
        ingredients: [],
    });
    ...
    const handleSubmit = async () => {
        ...
        const res = await fetch(`${process.env.NEXT_PUBLIC_URL}/api/meals/insertOne`, {
            method: "POST",
            headers: {
    ...
    return (
        <Appshell>
            <div className="flex justify-between pb-8">
                <h1 className="text-3xl font-semibold h">Nouveau plat {title}</h1>
            </div>
            <section className="flex flex-col gap-6">
                <div className="flex flex-row w-full gap-6 flex-wrap">
                    ...
                    <div>
                        ...
                        <Input onChange={(e) => handleChange("mealCode", e.target.value)} value={meal.mealCode} />
                    </div>
                    <div>
                ...
                <Table>
                    ...
                    <TableBody>
                        {meal?.ingredients?.map((ingredient: any) => (
                            <TableRow className="flex" key={ingredient._id}>
                                <TableCell className="w-[300px]">
                                    <TooltipProvider>
                                        <Tooltip>
                                            <TooltipTrigger className="h-full">
                                                <p className="w-[300px] text-left whitespace-nowrap truncate cursor-help">
                                                    {ingredient.name}
                                                </p>
                                            </TooltipTrigger>
                                            <TooltipContent>
            ...
            <Button onClick={handleSubmit}>Créer le plat {title}</Button>
        </Appshell>
    );
}
