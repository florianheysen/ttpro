                ...
                <Select onValueChange={field.onChange} defaultValue={field.value}>
                    <SelectTrigger>
                        <SelectValue className="w-full" placeholder={fieldConfigItem.inputProps?.placeholder}>
                            {field.value ? findItem(field.value)?.[1] : "Choisir une option..."}
                        </SelectValue>
                    </SelectTrigger>
                    <SelectContent>
...
function AutoFormObject<SchemaType extends z.ZodObject<any, any>>({
    ...
    return (
        <Accordion type="multiple" className="flex flex-wrap gap-5">
...
export function AutoFormSubmit({ children }: { children?: React.ReactNode }) {
    return <Button type="submit">{children ?? "Envoyer"}</Button>;
}
    ...
    return (
        <Form {...form}>
            <form
                ...
                className={cn("w-full max-w-2xl space-y-8", className)}
            >
